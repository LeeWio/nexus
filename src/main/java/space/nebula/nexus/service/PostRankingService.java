...
public class PostRankingService
{
	...
	{
		double score = Boolean.TRUE.equals(post.getIsFeatured()) ? positive(discoveryProperties.getFeaturedWeight(), 1_000) : 0;
	...
	{
		return source.getCategory() != null && candidate.getCategory() != null
				&& source.getCategory().getId() != null
				&& source.getCategory().getId().equals(candidate.getCategory().getId());
	}

	private boolean sameSeries(Post source, Post candidate)
	{
		return source.getSeries() != null && candidate.getSeries() != null
				&& source.getSeries().getId() != null
	...
	private long sharedTagCount(Post source, Post candidate)
	{
		...
		Set<Long> sourceTagIds = source.getTags().stream()
				.filter(tag -> tag.getId() != null)
				.map(space.nebula.nexus.entity.Tag::getId)
				.collect(Collectors.toSet());
		return candidate.getTags().stream()
				.filter(tag -> tag.getId() != null)
				.filter(tag -> sourceTagIds.contains(tag.getId()))
				.count();
	}
