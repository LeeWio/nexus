...
public class CommentGovernanceService {
	...
	public long countOpenReports(Long commentId) {
		Long count = jdbcTemplate.queryForObject(
				"SELECT COUNT(*) FROM blog_comment_report WHERE comment_id = ? AND status = ?", Long.class, commentId,
	...
	public ApiResponse<CommentGovernanceOverviewResponse> retrieveCommentGovernanceOverview() {
		...
		List<CommentGovernanceOverviewResponse.ModerationActionCount> moderationActionsLast7Days = retrieveModerationActionCounts(
				last7Days);
		...
		CommentGovernanceOverviewResponse overview = CommentGovernanceOverviewResponse.builder()
				...
				.oldestPendingAt(oldestPendingAt).commentsByStatus(toCommentStatusCounts(commentCounts))
				.reportsByStatus(toReportStatusCounts(reportCounts))
				.moderationActionsLast7Days(moderationActionsLast7Days).build();
	...
	private CommentReportResponse mapReport(ResultSet rs, int rowNum) throws SQLException {
		return CommentReportResponse.builder().commentId(rs.getLong("comment_id")).reporterId(rs.getLong("reporter_id"))
				.reporterUsername(rs.getString("reporter_username")).reporterNickname(rs.getString("reporter_nickname"))
				.reason(rs.getString("reason")).description(rs.getString("description"))
				.status(CommentReportStatus.valueOf(rs.getString("status"))).handledBy(rs.getString("handled_by"))
				.handledAt(toLocalDateTime(rs.getTimestamp("handled_at")))
				.resolutionNote(rs.getString("resolution_note")).postId(readNullableLong(rs, "post_id"))
				.postTitle(rs.getString("post_title")).parentId(readNullableLong(rs, "parent_id"))
	...
	private CommentRiskResponse mapRiskComment(ResultSet rs, int rowNum) throws SQLException {
		return CommentRiskResponse.builder().id(rs.getLong("id")).parentId(readNullableLong(rs, "parent_id"))
				.postId(readNullableLong(rs, "post_id")).postTitle(rs.getString("post_title"))
				.content(rs.getString("content")).username(rs.getString("username")).nickname(rs.getString("nickname"))
				.avatar(rs.getString("avatar")).status(CommentStatus.valueOf(rs.getString("status")))
				.reportsCount(rs.getLong("reports_count")).openReports(rs.getLong("open_reports"))
				.likesCount(rs.getLong("likes_count")).riskScore(rs.getLong("risk_score"))
				.createdAt(toLocalDateTime(rs.getTimestamp("created_at")))
				.editedAt(toLocalDateTime(rs.getTimestamp("edited_at"))).build();
	}
	...
	private List<CommentGovernanceOverviewResponse.ModerationActionCount> retrieveModerationActionCounts(
		...
		return jdbcTemplate.query(
				...
				(rs, rowNum) -> CommentGovernanceOverviewResponse.ModerationActionCount.builder()
						.action(CommentModerationAction.valueOf(rs.getString("action"))).count(rs.getLong("item_count"))
						.build(),
				since);
	}
	...
	private List<CommentGovernanceOverviewResponse.CommentStatusCount> toCommentStatusCounts(
		...
		for (CommentStatus status : CommentStatus.values()) {
			result.add(CommentGovernanceOverviewResponse.CommentStatusCount.builder().status(status)
					.count(counts.getOrDefault(status, 0L)).build());
		}
		return result;
	}
	...
	private List<CommentGovernanceOverviewResponse.ReportStatusCount> toReportStatusCounts(
		...
		for (CommentReportStatus status : CommentReportStatus.values()) {
			result.add(CommentGovernanceOverviewResponse.ReportStatusCount.builder().status(status)
					.count(counts.getOrDefault(status, 0L)).build());
		}
		return result;
	}
	...
	private CommentModerationLogResponse mapModerationLog(ResultSet rs, int rowNum) throws SQLException {
		return CommentModerationLogResponse.builder().id(rs.getLong("id")).commentId(rs.getLong("comment_id"))
				.postId(readNullableLong(rs, "post_id")).postTitle(rs.getString("post_title"))
				.parentId(readNullableLong(rs, "parent_id")).commentContent(rs.getString("comment_content"))
				...
				.moderatorUsername(rs.getString("moderator_username")).reason(rs.getString("reason"))
				.batchId(rs.getString("batch_id")).note(rs.getString("note"))
				.createdAt(toLocalDateTime(rs.getTimestamp("created_at"))).build();
	}
