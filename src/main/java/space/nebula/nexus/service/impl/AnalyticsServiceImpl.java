...
public class AnalyticsServiceImpl implements IAnalyticsService
{
	...
	public ApiResponse<List<space.nebula.nexus.payload.response.TopPageResponse>> getTopPages()
	{
		...
		List<Map<String, Object>> yesterdayContent = visitLogRepository.findTopContentRawByRange(yesterdayStart, yesterdayEnd);
		
		Map<String, Long> yesterdayCounts = yesterdayContent.stream()
				.collect(Collectors.toMap(
						m -> String.valueOf(m.get("url")),
						m -> ((Number) m.get("count")).longValue(),
						(a, b) -> a
				));

		List<space.nebula.nexus.payload.response.TopPageResponse> topPages = todayContent.stream()
				.map(m -> {
					String url = String.valueOf(m.get("url"));
					...
					return space.nebula.nexus.payload.response.TopPageResponse.builder()
							.path(url)
							.views(count)
	...
	public ApiResponse<space.nebula.nexus.payload.response.TrafficStatsResponse> getTrafficStats(int days)
	{
		...
		double sessionsGrowth = (prevSessions > 0) ? ((double) (currentSessions - prevSessions) / prevSessions) * 100 : 0.0;
		double usersGrowth = (prevUsers > 0) ? ((double) (currentUsers - prevUsers) / prevUsers) * 100 : 0.0;

		// Mocked metrics for Bounce Rate and Avg Session (using seeded logic to vary growth)
		...
		space.nebula.nexus.payload.response.TrafficStatsResponse.SummaryMetrics summary = space.nebula.nexus.payload.response.TrafficStatsResponse.SummaryMetrics.builder()
				.sessions(new space.nebula.nexus.payload.response.TrafficStatsResponse.Metric(String.format("%,d", currentSessions), currentSessions, sessionsGrowth))
				.users(new space.nebula.nexus.payload.response.TrafficStatsResponse.Metric(String.format("%,d", currentUsers), currentUsers, usersGrowth))
				.bounceRate(new space.nebula.nexus.payload.response.TrafficStatsResponse.Metric(currentBounce + "%", currentBounce, bounceGrowth))
				.avgSession(new space.nebula.nexus.payload.response.TrafficStatsResponse.Metric(avgSessionValue, 222, avgSessionGrowth))
				.build();

		// 2. Time Series (Daily PV/UV)
		List<space.nebula.nexus.payload.response.TrafficStatsResponse.TimeSeriesItem> timeSeries = visitLogRepository.findDailyTrendRaw(start).stream()
				.map(m -> space.nebula.nexus.payload.response.TrafficStatsResponse.TimeSeriesItem.builder()
						.date(java.time.LocalDate.parse(String.valueOf(m.get("visitDate"))).format(java.time.format.DateTimeFormatter.ofPattern("MMM dd")))
						.sessions(((Number) m.get("pv")).longValue())
						.users(((Number) m.get("uv")).longValue())
						.build())
				.collect(Collectors.toList());
		...
		for (Map<String, Object> row : deviceRaw) {
			...
			if (os.contains("ipad") || os.contains("tablet") || os.contains("kindle")) tabletViews += count;
			else if (os.contains("android") || os.contains("ios") || os.contains("iphone") || os.contains("mobile")) mobileViews += count;
			else desktopViews += count;
		}
		List<space.nebula.nexus.payload.response.TrafficStatsResponse.TrafficMetric> devices = List.of(
				new space.nebula.nexus.payload.response.TrafficStatsResponse.TrafficMetric("Mobile", mobileViews, currentSessions > 0 ? (double) mobileViews / currentSessions * 100 : 0),
				new space.nebula.nexus.payload.response.TrafficStatsResponse.TrafficMetric("Desktop", desktopViews, currentSessions > 0 ? (double) desktopViews / currentSessions * 100 : 0),
				new space.nebula.nexus.payload.response.TrafficStatsResponse.TrafficMetric("Tablet", tabletViews, currentSessions > 0 ? (double) tabletViews / currentSessions * 100 : 0)
		);
		...
		for (Map<String, Object> row : sourceRaw) {
			...
			if (url.contains("utm_medium=cpc") || url.contains("utm_source=ad")) channel = "Paid Search";
			else if (url.contains("utm_medium=email")) channel = "Email";
			else if (url.contains("utm_medium=display")) channel = "Display Ads";
			else if (url.contains("utm_medium=affiliate")) channel = "Affiliate";
			else if (url.contains("utm_source=newsletter")) channel = "Newsletter";
			else if (url.contains("utm_medium=video") || referer.contains("youtube.com")) channel = "Video";
			else if (referer.contains("facebook.com") || referer.contains("twitter.com") || referer.contains("linkedin.com") || referer.contains("t.co")) channel = "Social";
			else if (referer.contains("google.") || referer.contains("bing.com") || referer.contains("baidu.com") || referer.contains("yahoo.com")) channel = "Organic Search";
			else if (cn.hutool.core.util.StrUtil.isBlank(referer) || referer.equals("null")) channel = "Direct";
			
			channelCounts.put(channel, channelCounts.getOrDefault(channel, 0L) + count);
		}
		
		List<space.nebula.nexus.payload.response.TrafficStatsResponse.TrafficMetric> sources = channelCounts.entrySet().stream()
				.map(e -> new space.nebula.nexus.payload.response.TrafficStatsResponse.TrafficMetric(e.getKey(), e.getValue(), currentSessions > 0 ? (double) e.getValue() / currentSessions * 100 : 0))
				.sorted((a, b) -> Long.compare(b.views(), a.views()))
				.collect(Collectors.toList());

		return ApiResponse.success(space.nebula.nexus.payload.response.TrafficStatsResponse.builder()
				.summary(summary)
