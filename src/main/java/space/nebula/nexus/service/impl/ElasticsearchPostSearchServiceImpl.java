...
public class ElasticsearchPostSearchServiceImpl extends AbstractPostSearchService {
	...
	public void rebuildIndex() {
		...
		try {
			Map<String, Set<org.springframework.data.elasticsearch.core.index.AliasData>> aliases = indexOps
					.getAliases();
		...
		try {
			Map<String, Set<org.springframework.data.elasticsearch.core.index.AliasData>> oldIndices = indexOps
					.getAliases();
			oldIndices.keySet().stream().filter(idx -> !idx.equals(newIndexName))
		...
		log.info("Finished Blue-Green rebuild. Alias '{}' -> '{}'. Total indexed: {}", aliasName, newIndexName,
				totalIndexed);
	}
	...
	public ApiResponse<PageResult<PostDocument>> searchPosts(String keyword, Pageable pageable) {
		...
		var query = NativeQuery.builder()
				.withQuery(q -> q.multiMatch(m -> m.fields("title^8", "summary^3", "content^1").query(keyword)
						.fuzziness("AUTO").type(TextQueryType.BestFields)))
				.withHighlightQuery(new HighlightQuery(new Highlight(List.of(new HighlightField("title"),
						new HighlightField("summary"), new HighlightField("content"))), PostDocument.class))
				.withPageable(pageable).build();

		SearchHits<PostDocument> searchHits = elasticsearchOperations.search(query, PostDocument.class);

		List<PostDocument> documents = searchHits.getSearchHits().stream().map(hit -> {
			...
			// 2. Highlight Summary if present, otherwise fall back to Content highlight
			// fragment
		...
		return ApiResponse.success(PageResult
				.of(new org.springframework.data.domain.PageImpl<>(documents, pageable, searchHits.getTotalHits())));
	}
	...
	public ApiResponse<List<String>> getSearchSuggestions(String keyword) {
		...
		var query = NativeQuery.builder().withQuery(q -> q.matchPhrasePrefix(m -> m.field("title").query(keyword)))
				.withPageable(org.springframework.data.domain.PageRequest.of(0, 10)).build();

		SearchHits<PostDocument> hits = elasticsearchOperations.search(query, PostDocument.class);
		List<String> suggestions = hits.getSearchHits().stream().map(hit -> hit.getContent().getTitle()).distinct()
				.toList();
	...
	protected List<QuickSearchResponse.SearchResultItem> searchQuickPosts(String keyword) {
		var query = NativeQuery.builder()
				...
				.withPageable(org.springframework.data.domain.PageRequest.of(0, 5)).build();

		SearchHits<PostDocument> hits = elasticsearchOperations.search(query, PostDocument.class);
		return hits.getSearchHits().stream()
				.map(hit -> new QuickSearchResponse.SearchResultItem(hit.getContent().getId(),
						hit.getContent().getTitle(), "/posts/" + hit.getContent().getSlug()))
				.toList();
	}

	@Override
	protected void searchPostsProfessional(String keyword, List<UnifiedSearchResponse.SearchGroup> groups) {
		var query = NativeQuery.builder().withQuery(
				q -> q.multiMatch(m -> m.fields("title^8", "summary^3", "content^1").query(keyword).fuzziness("AUTO")))
				.withHighlightQuery(new HighlightQuery(new Highlight(List.of(new HighlightField("title"),
						new HighlightField("summary"), new HighlightField("content"))), PostDocument.class))
				.withPageable(org.springframework.data.domain.PageRequest.of(0, 5)).build();
		...
		List<UnifiedSearchResponse.SearchResultItem> items = searchHits.getSearchHits().stream().map(hit -> {
			...
			var highlights = hit.getHighlightFields();
			if (highlights.containsKey("title"))
				displayTitle = String.join("", highlights.get("title"));
			if (highlights.containsKey("summary")) {
			...
			return UnifiedSearchResponse.SearchResultItem.builder().id("post:" + p.getId()).title(displayTitle)
					.subtitle(p.getPublishedAt() != null ? p.getPublishedAt().toLocalDate().toString() : "")
					.description(displayDesc).url("/post/" + p.getSlug()).icon("book-text").iconColor("#3b82f6")
					.type("POST").score((double) hit.getScore()).build();
		}).toList();
