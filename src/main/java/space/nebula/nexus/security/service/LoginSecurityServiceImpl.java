...
public class LoginSecurityServiceImpl implements LoginSecurityService {
    ...
    public void validateLoginLock(String username) {
        ...
        if (fails >= MAX_LOGIN_FAILURES) {
            ...
            throw new BusinessException(403, String.format(
                "Account is locked due to too many failed attempts. Please try again in %d minutes.", 
                minutesLeft
            ));
        }
    }
    ...
    public void recordLoginFailure(String username) {
        ...
        if (count != null && count >= MAX_LOGIN_FAILURES) {
            log.error("User {} has been locked for {} minutes after {} failed attempts", 
                    username, LOCK_DURATION_MINUTES, count);
        }
    }
