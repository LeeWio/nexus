...
public record AnalyticsOverviewResponse(@Schema(description = "Total Page Views today") long todayPv,
	...
	@Builder
	public record VisitTrendItem(@Schema(description = "Date in YYYY-MM-DD format") String date,
			@Schema(description = "Page Views") long pv,
			@Schema(description = "Unique Visitors") long uv) implements Serializable {
	}

	@Builder
	public record TopContentItem(@Schema(description = "Content URL or Title") String title,
			@Schema(description = "Page path") String url,
			@Schema(description = "View count") long count) implements Serializable {
	}
}
