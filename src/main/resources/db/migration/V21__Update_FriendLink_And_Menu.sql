...
ALTER TABLE `blog_friend_link` ADD COLUMN `email` VARCHAR(100) AFTER `description`;
ALTER TABLE `blog_friend_link` ADD COLUMN `status` VARCHAR(20) NOT NULL DEFAULT 'APPROVED' AFTER `email`;

-- Update sys_menu
ALTER TABLE `sys_menu` ADD COLUMN `is_visible` BOOLEAN NOT NULL DEFAULT TRUE;
ALTER TABLE `sys_menu` ADD COLUMN `is_public` BOOLEAN NOT NULL DEFAULT FALSE;
