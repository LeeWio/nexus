...
('1.1.1.1', '/', 'Mozilla', DATE_SUB(NOW(), INTERVAL 14 DAY), DATE_SUB(NOW(), INTERVAL 14 DAY), 0),
('1.1.1.2', '/post/java-21-features', 'Mozilla', DATE_SUB(NOW(), INTERVAL 14 DAY), DATE_SUB(NOW(), INTERVAL 14 DAY), 0),
('1.1.1.3', '/post/welcome-to-nexus', 'Mozilla', DATE_SUB(NOW(), INTERVAL 14 DAY), DATE_SUB(NOW(), INTERVAL 14 DAY), 0);

-- 10 days ago (Spike)
INSERT INTO `sys_visit_log` (ip_address, request_url, user_agent, visit_time, created_at, is_deleted) 
SELECT CONCAT('2.2.2.', id), '/post/modern-frontend-react', 'Chrome', DATE_SUB(NOW(), INTERVAL 10 DAY), DATE_SUB(NOW(), INTERVAL 10 DAY), 0 
...
-- 7 days ago
INSERT INTO `sys_visit_log` (ip_address, request_url, user_agent, visit_time, created_at, is_deleted) 
SELECT CONCAT('3.3.3.', id), '/', 'Safari', DATE_SUB(NOW(), INTERVAL 7 DAY), DATE_SUB(NOW(), INTERVAL 7 DAY), 0 
FROM (SELECT 1 as id UNION SELECT 2 UNION SELECT 3 UNION SELECT 4 UNION SELECT 5) as t;

-- 5 days ago
INSERT INTO `sys_visit_log` (ip_address, request_url, user_agent, visit_time, created_at, is_deleted) 
SELECT CONCAT('4.4.4.', id), '/post/building-search-elasticsearch', 'Mozilla', DATE_SUB(NOW(), INTERVAL 5 DAY), DATE_SUB(NOW(), INTERVAL 5 DAY), 0 
...
-- 3 days ago
INSERT INTO `sys_visit_log` (ip_address, request_url, user_agent, visit_time, created_at, is_deleted) 
SELECT CONCAT('5.5.5.', id), '/project/nexus-cms', 'Edge', DATE_SUB(NOW(), INTERVAL 3 DAY), DATE_SUB(NOW(), INTERVAL 3 DAY), 0 
FROM (SELECT 1 as id UNION SELECT 2 UNION SELECT 3 UNION SELECT 4 UNION SELECT 5 UNION SELECT 6) as t;

-- 2 days ago
INSERT INTO `sys_visit_log` (ip_address, request_url, user_agent, visit_time, created_at, is_deleted) 
SELECT CONCAT('6.6.6.', id), '/post/java-21-features', 'Chrome', DATE_SUB(NOW(), INTERVAL 2 DAY), DATE_SUB(NOW(), INTERVAL 2 DAY), 0 
FROM (SELECT 1 as id UNION SELECT 2 UNION SELECT 3 UNION SELECT 4) as t;

-- Yesterday (High volume for comparison)
INSERT INTO `sys_visit_log` (ip_address, request_url, user_agent, visit_time, created_at, is_deleted) 
SELECT CONCAT('7.7.7.', id), '/', 'Mozilla', DATE_SUB(NOW(), INTERVAL 1 DAY), DATE_SUB(NOW(), INTERVAL 1 DAY), 0 
