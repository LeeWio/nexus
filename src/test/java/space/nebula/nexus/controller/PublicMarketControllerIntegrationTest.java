...
class PublicMarketControllerIntegrationTest {
	...
	void searchStocks_ReturnsList() throws Exception {
		...
		mockMvc.perform(get("/api/v1/public/market/stocks/search")
						.param("keyword", "茅台")
	...
	void getStockTrend_ReturnsData() throws Exception {
		StockTrendResponse mockTrend = StockTrendResponse.builder()
				.name("贵州茅台")
				.symbol("sh600519")
				.current(new BigDecimal("1610.00"))
				.changePct(new BigDecimal("1.26"))
				.isOpen(true)
				.trendPoints(List.of(
						new StockTrendResponse.TrendPoint("2026-08-20", new BigDecimal("1590.00")),
		...
		mockMvc.perform(get("/api/v1/public/market/stocks/sh600519/trend")
						.param("period", "1M")
						.contentType(MediaType.APPLICATION_JSON))
				.andExpect(status().isOk())
				.andExpect(jsonPath("$.code").value(200))
